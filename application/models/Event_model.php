...
class Event_model extends CI_Model
{
    ...
    function getAll()
    {
        $this->db->select('e.id, e.title, e.image, e.slug,  e.type, start_time, e.end_time, 
                            e.status, e.location, e.tahun_karya, p.name as publisher_name,p.image as publisher_image, 
                 ...
                 ->where('e.end_time >', date('Y-m-d'))
                 ->limit(10)
                 ->order_by('e.start_time', 'ASC');
        return $this->db->get()->result();
    }

    function cariEvent($keyword = null, $kategori = null, $kota = null, $seniman = null)
    {
        $this->db->select('e.id, e.title, e.image as event_image, e.slug,  start_time, e.end_time,  
                           e.type, e.city, e.status, e.location, e.tahun_karya, p.name as publisher_name, p.image,
                           ec.name as category_name, c.name as city_name')
                 ->from('events e')
                 ->join('publishers p','e.publisher = p.id')
                 ->join('event_category ec','e.category = ec.id')
    ...
    function getEventBySeniman($publisher)
    {
        $this->db->select('*')
                 ->from('event e')
                 ->join('publisher p','e.event_publisher = p.publisher_id')
                 ->join('city ci','e.event_city = ci.city_id')
                 ->join('category c','e.event_category = c.category_id')
