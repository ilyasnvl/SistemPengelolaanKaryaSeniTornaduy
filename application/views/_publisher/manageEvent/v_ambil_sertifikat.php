    ...
    <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
        ...
        <div class="collapse navbar-collapse justify-content-md-end" id="navbarsExample10">
            <ul class="navbar-nav">
                ...
                <li class="nav-item">
                    <a class="nav-link active" href="<?= site_url('manage/'.$event_id.'/sertifikat') ?>">
    ...
    <div class="container" style="margin-top:30px">
        <div class="row">
            <div class="col-12" style="margin-bottom: 20px; margin-left:15px">
                <h2>Scan Pengambilan Sertifikat</h2>
            </div>
        </div>
        <div class="row">
            <div class="col-4">
                    ...
                    <div class="col-11">
                        <div class="card shadow">
                            <div class="card-body text-center">
                                ...
                                <hr>
                                <form action="<?= site_url('Publisher/ManageEvent/scan_ambil_sertifikat/'.$event_id) ?>" method="post">
                                    <small>Masukan QR Code yang terdapat di tiket</small> <br> <br>
                                    <input type="text" name="qrcode" id="qrcode" class="form-control" placeholder="misal: QRC0d3" autofocus required>
                                    <br>
                                    <button type="submit" name="submit" class="btn btn-primary btn-block">SUBMIT</button>
                                </form>
                            </div>
                            <?php 
                            if ($this->session->userdata('sertifikat-msg') <> '') { ?>
                                <div class="card-footer">
                                    <div class="alert alert-danger" role="alert">
                                        <?= $this->session->userdata('sertifikat-msg') ?>
                                    </div>
                                </div>
                            <?php } ?>
                        </div>
            ...
            <div class="col-8">
                <table class="table table-striped table-bordered table-hover" id="kedatangan">
                    ...
                    <tbody>
                        <?php 
                        $start = 0;
                        foreach ($sertifikat_data as $row) { ?>
                            <tr>
                                <td style="text-align: center;width:50px"><?= ++$start ?></td>
                                <td style="width:400px"><?= $row->name ?></td>
                                <td style="text-align: center"><?= date('H:i - d M Y',strtotime($row->certificate_time)) ?></td>
                            </tr>
                        <?php } ?>
                    </tbody>
                </table>
